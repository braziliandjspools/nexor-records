...
import { ArrowLeft, ArrowRight, Home, Search, Filter, ChevronDown, HelpCircle, Lock, Unlock, Copy, Check, AlertTriangle, Computer, Folder, KeyRound, Loader2 } from "lucide-react"
...
const acervo2023Data = {
  ...
  prevAcervo: null, // Pode ser preenchido dinamicamente no futuro
  nextAcervo: null,
};
...
export default function Acervos2023Page() {
  ...
  const groupedItems = useMemo(() => {
    return filteredItems.reduce((acc, item) => {
        const date = item.date || "Sem Data"; // Garante um agrupamento padrão
  ...
  // CORREÇÃO: A variável foi renomeada de displayedItems para displayedFolders para consistência
  ...
  return (
    <div className="space-y-8 pb-24">
      ...
      <section>
                        ...
                        {items.map((item) => {
                            ...
                            return (
                                <div key={item.id} className="flex flex-col sm:flex-row items-center sm:gap-2">
                                    <div className="relative flex-grow w-full">
                                        <button onClick={() => handleOpenLink(item.id, item.link)} className="bg-gradient-to-r from-black/80 to-black/60 p-4 rounded-md border border-green-600/20 hover:border-green-600/70 flex items-center gap-3 w-full">
        ...
        {/* CORREÇÃO: A lógica do botão de carregar mais agora usa 'filteredItems' */}
      ...
      <footer className="fixed bottom-0 left-0 w-full bg-black/80 backdrop-blur-sm border-t border-green-600/30 p-4 z-20">
        <div className="container mx-auto flex justify-center items-center gap-2 sm:gap-4">
            {/* A navegação do rodapé pode ser tornada dinâmica no futuro */}
